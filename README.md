# Getting Started with ROS 2: From Basics to Manipulator Simulation

[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](LICENSE)
[![ROS 2 Jazzy](https://img.shields.io/badge/ROS%202-Jazzy-blue)](https://docs.ros.org/en/jazzy/)

Complete hands-on course for learning ROS 2 from scratch to robot simulation and motion planning.

## Course Overview

**Duration:** 4 weeks  
**Format:** 9 hands-on workshops  
**Level:** Beginner to Intermediate

### What You'll Learn

- Linux fundamentals and terminal usage
- Python programming for robotics
- ROS 2 communication (Topics, Services, Actions)
- Robot description with URDF
- 3D visualization with RViz2
- Physics simulation with Gazebo
- Sensor integration and data processing
- Motion planning with MoveIt2

## Course Structure

### [Fundamentals](workshops/fundamentals/) - Week 1 (2 sessions)
1. [Linux Basics](workshops/fundamentals/linux-basics/)
2. [Python for Robotics](workshops/fundamentals/python-basics/)

### [ROS 2 Core](workshops/ros2-core/) - Week 2 (2 sessions)
3. [Nodes and Topics](workshops/ros2-core/nodes-topics/)
4. [Services and Actions](workshops/ros2-core/services-actions/)

### [Visualization](workshops/visualization/) - Week 3 (1 session)
5. [URDF and RViz2](workshops/visualization/urdf-rviz/)

### [Simulation](workshops/simulation/) - Week 4 (3 sessions)
6. [Gazebo Basics](workshops/simulation/gazebo-basics/)
7. [Gazebo Sensors + RViz2](workshops/simulation/gazebo-sensors/)
8. [MoveIt2 Basics](workshops/simulation/moveit2-basics/)

## Getting Started

### Prerequisites
- Ubuntu 24.04 (VM or Dual Boot)
- 8GB RAM minimum
- Internet connection
- No prior ROS experience required

### Installation
Follow our detailed [Installation Guide](assets/resources/installation-guides/COURSE_SETUP.md)

## Repository Structure
```
workshops/          -> All workshop content
├── fundamentals/   -> Linux & Python basics (2 workshops)
├── ros2-core/      -> ROS 2 communication (2 workshops)
├── visualization/  -> URDF & RViz2 (1 workshop)
└── simulation/     -> Gazebo & MoveIt2 (3 workshops)

assets/             -> Supporting materials
├── images/         -> Diagrams and screenshots
├── gifs/           -> Animated demonstrations
├── resources/      -> Guides and references
├── certification/  -> Badge system
└── config/         -> Repository configuration
```

## Additional Resources

- [Course Syllabus](assets/resources/references/SYLLABUS.md)
- [Cheat Sheets](assets/resources/cheat-sheets/)
- [Installation Guides](assets/resources/installation-guides/)
- [Troubleshooting](assets/resources/installation-guides/troubleshooting.md)

## Certification

Upon completion, receive an official course badge.  
[Learn more](assets/certification/BADGE_SYSTEM.md)

## Contributing

Contributions welcome! See [Contributing Guidelines](CONTRIBUTING.md)

## Support

- **Issues:** [Report a problem](../../issues)
- **Discussions:** [Ask questions](../../discussions)

## License

Apache License 2.0 - see [LICENSE](LICENSE)

---

**Happy Learning!**
